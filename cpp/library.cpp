                       ...
                       std::size_t batchSize, std::size_t sampleCount)
{
    ...
    if (batchSize == 0 || sampleCount == 0 || sampleCount > batchSize) {
        input.close();
        return;
    }
    ...
    auto flush = [&] {
        for (std::size_t i = 0; i < sampleCount; ++i) {
            const std::size_t index = i * batchSize / sampleCount;
            if (index >= batch.size()) // truncated final partial batch
                break;
            if (!output.push(std::move(batch[index]))) {
                input.close(); // downstream gave up; stop the upstream producer too
                return false;
            }
        }
