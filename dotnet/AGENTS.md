...
- Build/run: `dotnet build` / `dotnet run` in `DataGenUI/`. Start the server first, e.g.
  `cpp/build/frame_server 15071 0.001 5 1 1 300` — the last argument is the idle timeout in
  seconds and it ticks between user clicks, so keep it generous for human-paced review.
